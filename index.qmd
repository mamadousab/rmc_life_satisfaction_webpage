...
cantril2 <- cantril1 %>%
  mutate(absolute_change = round(`2023` - `2011`, 2),
         relative_change = percent((`2023` - `2011`) / `2011`, accuracy = 0.01))
...
# Create line chart
cantril3 <- cantril2 %>% 
  filter(Country %in% c("Guinea", "Brazil", "Uganda", "Australia"))

# Reshape data from wide to long format
cantril_long <- cantril3 %>%
  pivot_longer(cols = c(`2011`, `2023`), names_to = "Year", values_to = "Score")

# Convert Year to numeric for proper ordering on the x-axis
cantril_long$Year <- as.numeric(cantril_long$Year)

# Create the line chart
ggplot(cantril_long, aes(x = Year, y = Score, color = Country, group = Country)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  labs(title = "Cantril Ladder Score Over Time by Country",
       x = "Year",
...
cantril_plot <- ggplot(cantril_long, aes(x = Year, y = Score, color = Country, group = Country)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  labs(title = "Cantril Ladder Score Over Time by Country",
       x = "Year",
